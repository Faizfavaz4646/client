...
export default function WorkspaceLayout({ children }: { children: React.ReactNode }) {
  ...
  const isPrivileged = user?.organizations?.some(org => String(org.orgId) === String(activeOrgId) && (org.role === 'admin' || org.role === 'owner')) ||
    user?.workspaces?.some(w => (w.workspaceId === activeWorkspaceId || w._id === activeWorkspaceId) && (w.role === 'admin' || w.role === 'owner'));
  const isOrgFounder = !!user?.organizations?.some(org => String(org.orgId) === String(activeOrgId) && (org.role === 'admin' || org.role === 'owner'));
